 ...
 */
export async function handleGetUserStats(params?: { startDate?: string; endDate?: string }): Promise<UserStats> {
  // Simulate API delay
 ...
 */
export async function handleUpdateUserTags(id: string, payload: UserTagUpdatePayload): Promise<any> {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 500 + Math.random() * 300));
...
export async function handleExportUsers(params: UserQueryParams): Promise<UserExportResponse> {
  ...
  const csvContent = "id,nickname,email,phone,kycStatus,vipLevel,status\n" +
