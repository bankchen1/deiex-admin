 ...
 */
export async function handleListWithdrawals(params: WithdrawalQueryParams): Promise<WithdrawalListResponse> {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 300 + Math.random() * 200));
  ...
  if (params.currency) {
    filteredWithdrawals = filteredWithdrawals.filter(withdrawal => withdrawal.currency === params.currency);
  }
  
  if (params.chain) {
    filteredWithdrawals = filteredWithdrawals.filter(withdrawal => withdrawal.chain === params.chain);
  }
  
  if (params.userId) {
    filteredWithdrawals = filteredWithdrawals.filter(withdrawal => withdrawal.userId === params.userId);
  }
 ...
 */
export async function handleListWalletAddresses(params: WalletAddressQueryParams): Promise<WalletAddressListResponse> {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 300 + Math.random() * 200));
  ...
  return {
    ...
    auditTrail: [
      ...
      },
      {
        ...
        details: deposit.status === "completed" ? "Deposit completed and credited" : `Deposit status updated to ${deposit.status}`
      }
    ]
  };
}
  ...
  return {
    ...
    transactionHistory: [
      ...
      },
      {
        ...
        timestamp: withdrawal.approvals.length > 0 ? withdrawal.approvals[0].timestamp : withdrawal.createdAt,
      },
      {
        id: "th_003",
    ...
    auditTrail: [
      ...
      {
        id: "at_002",
        action: withdrawal.status === "completed" ? "WITHDRAWAL_PROCESSED" : "WITHDRAWAL_STATUS_UPDATED",
        timestamp: withdrawal.completedAt || new Date().toISOString(),
        by: withdrawal.status === "completed" ? "system" : "risk_engine",
        details: withdrawal.status === "completed" ? "Withdrawal processed and sent" : `Withdrawal status updated to ${withdrawal.status}`
      }
    ]
  };
}
 ...
 */
export async function handleUpdateDepositNotes(id: string, payload: UpdateDepositNotesPayload): Promise<any> {
  // Simulate API delay
 ...
 */
export async function handleApproveWithdrawal(id: string, payload: ApproveWithdrawalPayload): Promise<any> {
  // Simulate API delay
  ...
  mockData.withdrawals[withdrawalIndex] = {
    ...
    status: mockData.withdrawals[withdrawalIndex].status === "pending" ? "approved" : mockData.withdrawals[withdrawalIndex].status
  };
 ...
 */
export async function handleRejectWithdrawal(id: string, payload: RejectWithdrawalPayload): Promise<any> {
  // Simulate API delay
  await new Promise(resolve => setTimeout(resolve, 600 + Math.random() * 400));
