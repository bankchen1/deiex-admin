...
// Selection
const selectedOrders = ref<FuturesOrder[]>([])

// Detail Drawer
const detailDrawerOpen = ref(false)
const selectedOrder = ref<FuturesOrder | null>(null)
...
function handleSelectionChange(rows: FuturesOrder[]) {
  selectedOrders.value = rows
}

async function handleViewDetail(order: FuturesOrder) {
  ...
  if (order.id) {
    ...
    try {
      await ordersStore.fetchFuturesOrderById(order.id)
      selectedOrder.value = ordersStore.currentFuturesOrder
    } finally {
...
async function handleBatchExport() {
  ...
  await ordersStore.exportFuturesOrders({
    ...appliedFilters.value,
